...
**Description**
    ...
    switch, it would be more appropriate to check to see if the port was in use (online) first. The brcddb library
    was intended for being able to support that type of validation. As of this publication date, an example on how to
    create and modify switches with additional checking using the brcddb library was planned but not yet developed.
...
Version Control::
    ...
    +-----------+---------------+-----------------------------------------------------------------------------------+
    | 3.0.1     | 31 Dec 2020   | Updated comments only                                                             |
    +-----------+---------------+-----------------------------------------------------------------------------------+
"""
...
def parse_args():
    ...
    if _DEBUG:
        ...
        return args.ip, args.id, args.pw, args.s, args.fid, args.name, args.did, args.idid, args.xisl, args.base, \
               args.ficon, args.ports, args.ge_ports, args.se, args.pe, args.echo, args.d
